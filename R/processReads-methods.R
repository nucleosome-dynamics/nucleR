setMethod("processReads", signature(data="AlignedRead"), 
	function(data, type="single", fragmentLen, trim, ...) {	
		...
		if(type=="single")
		{
			...
			shift  = floor((fragmentLen-sr_len)/2) #Floor, then we will add 1 if needed
		  ...
		  new_levels[which(levels(strand) == "*")] = 0
			...
			#If the fragment_length - read_length is odd and negative strand, add 1 extra base
			correct = rep(0, length(shift))
			correct[(strand == -1) & ((fragmentLen-sr_len) %% 2 == 1)] = 1
	
			start=position(data)+((shift+correct)*strand)
			res = RangedData(ranges=IRanges(start=start, width=sr_len), space=as.character(chromosome(data)))	
		}
		...
		else if(type=="paired")
		{
			...
			if(!missing(trim)) start = start + (sr_len[selection] / 2 - floor(trim/2))

			width = sr_len[selection]
			if(!missing(trim)) width = trim
			res = RangedData(ranges=IRanges(start=start, width=width), space=space)
		}

		################################################################################
		else stop("type must be 'single' for single-ended data or 'paired' for paired-ended")
...
setMethod("processReads", signature(data="RangedData"),
  function(data, type="single", fragmentLen, trim, ...) {
    ...
    if(type=="single")
    {
      ...
      res = RangedData(ranges=IRanges(start=start, width=sr_len), space=space(data))
    }
    ...
    else if(type=="paired")
    {
      ...
      #Normal start or trimmed version if requested
      start = start(data)
      if(!missing(trim)) start = start + ((sr_len/2) - floor(trim/2))

      width = sr_len
			if(!missing(trim)) width=trim

      res = RangedData(ranges=IRanges(start=start, width=width), space=space(data))
    }
