setMethod(
    ...
    function(peaks, data, threshold=0, scores=NULL, start=1, end=length(data),
        ...
        if (!is.null(scores))
            text(peaks, data[as.character(peaks)],
...
setMethod(
    ...
    function(peaks, data, threshold=0, scores=NULL, start=1, end=length(data),
        ...
        if (!is.null(scores) & "nmerge" %in% names(scores)) {
            ...
            if (length(p1) > 0) {
                rect(start(p1), ybot + pc, end(p1), ybot + pc * rect.thick,
                     lwd=rect.lwd, col=col.points, border=rect.border)
            }
            ...
            ybot <- ybottom[scores$nmerge > 1]
            if(length(p2) > 0) {
                rect(start(p2), ybot + pc, end(p2), ybot + pc * rect.thick,
                     lwd=rect.lwd, col=col.points, border=rect.border,
                     density=30)
            }

        } else {  # All are normal calls
            rect(start(peaks), ybottom + pc, end(peaks),
                 ybottom + pc * rect.thick, lwd=rect.lwd, col=col.points,
                 border=rect.border)
        }
